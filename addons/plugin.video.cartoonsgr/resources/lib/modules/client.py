...
import urllib.request, urllib.parse, urllib.error, urllib.request, urllib.error, urllib.parse, urllib.parse, html.parser
from . import cache
...
def request(url, close=True, redirect=True, error=False, proxy=None, post=None, headers=None, mobile=False, limit=None,
    ...
    # try:
        ...
        if proxy is not None:
            ...
            handlers += [urllib.request.ProxyHandler({'http':'{0}'.format(proxy)}), urllib.request.HTTPHandler]
...
def randomagent():
    ...
    BR_VERS = [['%s.0' % i for i in range(18, 43)], ['37.0.2062.103', '37.0.2062.120', '37.0.2062.124', '38.0.2125.101',
...
def spoofer(_agent=True, age_str=randomagent(), referer=False, ref_str=''):
    ...
    if _agent and referer:
        return '|User-Agent=' + urllib.parse.quote_plus(age_str) + '&Referer=' + urllib.parse.quote_plus(ref_str)
    elif _agent:
        return '|User-Agent=' + urllib.parse.quote_plus(age_str)
    elif referer:
        return '|Referer=' + urllib.parse.quote_plus(ref_str)
...
def cfcookie(netloc, ua, timeout):
    try:
        ...
        req = urllib.request.Request(netloc, headers=headers)
        ...
        try:
            ...
            urllib.request.urlopen(request, timeout=int(timeout))
        except:
            pass
