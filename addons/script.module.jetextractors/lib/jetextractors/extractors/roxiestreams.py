...
from ..endpoints import DOMAINS_PREFERRED, ROXIESTREAMS_PREFERRED
...
class RoxieStreams(JetExtractor):
    ...
    def _resolve_raw_page(self, raw_url: str, button_text: str = None) -> Optional[JetLink]:
        ...
        try:
            ...
            if resp.status_code != 200:
                debug_log(f"[RoxieStreams] _resolve_raw_page bad status {resp.status_code} for {raw_url}", xbmc.LOGWARNING)
                return None
            html = resp.text
            debug_log(f"[RoxieStreams] _resolve_raw_page fetched {raw_url} (len {len(html)})", xbmc.LOGINFO)
        except Exception as e:
            debug_log(f"[RoxieStreams] _resolve_raw_page fetch failed for {raw_url}: {e}", xbmc.LOGWARNING)
            return None

        # Shaka Player manifest load: player.load("https://.../cenc.mpd")
        load_match = re.search(r'player\.load\(\s*[\'"]([^\'"]+\.(?:mpd|m3u8)[^\'"]*)[\'"]', html)
        if not load_match:
            debug_log(f"[RoxieStreams] _resolve_raw_page: no player.load() manifest found in {raw_url}", xbmc.LOGWARNING)
            return None
        stream_url = load_match.group(1)
        debug_log(f"[RoxieStreams] _resolve_raw_page: found manifest {stream_url}", xbmc.LOGINFO)
        ...
        if ck_match:
            ...
            if kid_key:
                ...
                debug_log(f"[RoxieStreams] _resolve_raw_page: ClearKey DRM detected (kid={kid_hex}, key={key_hex})", xbmc.LOGINFO)
        ...
        if stream_url.endswith(".mpd") or ".mpd?" in stream_url:
            # Proxy DASH manifest through local proxy to ensure correct headers
            ...
            debug_log(f"[RoxieStreams] _resolve_raw_page: inputstream dict = {_json.dumps(inputstream.to_dict())}", xbmc.LOGINFO)
            link = JetLink(
                address=patched_url,
                name=button_text,
                resolveurl=False,
        ...
        debug_log(f"[RoxieStreams] _resolve_raw_page: returning link for {button_text or raw_url}", xbmc.LOGINFO)
        return link
    ...
    def _patch_m3u8_segments(self, stream_url: str, headers: dict, fallback_urls: list = None):
        try:
            ...
            debug_log(f"[RoxieStreams] Failed to register proxy: {e}", xbmc.LOGWARNING)
            return stream_url, None

    def _build_domain_fallbacks(self, primary_url: str, subdomain: str, stream_path: str, domains_list: List[str]) -> List[str]:
        fallbacks = []
            ...
            for url in urls:
                try:
                    ...
                    if resp.status_code == 200:
                        ...
                        if preferred:
                            # debug_log(f"[RoxieStreams] Loaded {len(preferred)} preferred domains from {url}", xbmc.LOGINFO)
                            debug_log(f"[RoxieStreams] Loaded {len(preferred)} preferred domains from server", xbmc.LOGINFO)
                            return preferred
                except Exception as e:
                    debug_log(f"[RoxieStreams] Failed to fetch from {url}: {e}", xbmc.LOGWARNING)
        except Exception as e:
            debug_log(f"[RoxieStreams] Failed to fetch preferred domains: {e}", xbmc.LOGERROR)
        fallback = ["shadow-ran.online"]
        debug_log(f"[RoxieStreams] Using fallback preferred domains: {fallback}", xbmc.LOGINFO)
        return fallback
    ...
    def _pick_working_stream_url(self, subdomain: str, stream_path: str, domains_list: List[str], button_text: str) -> str:
        for domain in domains_list:
            ...
            debug_log(f"[RoxieStreams] Button '{button_text}' using URL: {test_url}", xbmc.LOGINFO)
            return test_url
        fallback = f"https://{subdomain}.{domains_list[0]}/{stream_path}"
        debug_log(f"[RoxieStreams] Button '{button_text}' fallback URL: {fallback}", xbmc.LOGINFO)
        return fallback
    ...
    def get_items(self, params: Optional[dict] = None, progress: Optional[JetExtractorProgress] = None) -> List[JetItem]:
        ...
        debug_log(f"[RoxieStreams] Found {len(nav_links)} nav-links", xbmc.LOGINFO)
        ...
        for nav_link in nav_links:
            ...
            if not href or href == "#" or "discord" in href.lower() or not href.startswith("http"):
                debug_log(f"[RoxieStreams] Skipping nav-link: {league} ({href})", xbmc.LOGINFO)
                continue
            
            debug_log(f"[RoxieStreams] Processing league: {league} - {href}", xbmc.LOGINFO)
            ...
            try:
                ...
                debug_log(f"[RoxieStreams] Found {len(all_links)} td elements for {league}", xbmc.LOGINFO)
                ...
                for td in all_links:
                    ...
                    if "roxiestreams" in event_href and "streams" in event_href:
                        ...
                        debug_log(f"[RoxieStreams] Added event: {title} -> {event_href}", xbmc.LOGINFO)
            except Exception as e:
                debug_log(f"[RoxieStreams] Error processing league {league}: {e}", xbmc.LOGERROR)
                
            if self.progress_update(progress, league):
                break
        
        debug_log(f"[RoxieStreams] Total items found: {len(items)}", xbmc.LOGINFO)
        return items
    ...
    def get_links(self, url: JetLink) -> List[JetLink]:
        ...
        try:
            ...
            if buttons:
                debug_log(f"[RoxieStreams] Found {len(buttons)} stream buttons", xbmc.LOGINFO)
                ...
                for domains_file in ('domainsz30.txt', 'domainsz25.txt', 'domains.txt'):
                    try:
                        ...
                        debug_log(f"[RoxieStreams] Fetching domains from: {domains_url}", xbmc.LOGINFO)
                        ...
                        if domains_response.status_code == 200:
                            ...
                            debug_log(f"[RoxieStreams] Loaded {len(domains_list)} domains from {domains_file}: {domains_list}", xbmc.LOGINFO)
                            if domains_list:
                                break
                    except Exception as e:
                        debug_log(f"[RoxieStreams] Failed to fetch {domains_file}: {e}", xbmc.LOGERROR)
                
                if not domains_list:
                    domains_list = ['shadow-ran.online']
                    debug_log(f"[RoxieStreams] Using fallback domains: {domains_list}", xbmc.LOGINFO)
                ...
                if domain_from_page:
                    ...
                    if extracted_domain not in domains_list:
                        ...
                        debug_log(f"[RoxieStreams] Extracted domain from page: {extracted_domain}", xbmc.LOGINFO)
                ...
                debug_log(f"[RoxieStreams] Final domains list: {domains_list}", xbmc.LOGINFO)
                
                # Keep domains in order - first is tried first
                debug_log(f"[RoxieStreams] Using domains: {domains_list}", xbmc.LOGINFO)
                ...
                for script in scripts:
                    ...
                    if subdomain_match:
                        ...
                        debug_log(f"[RoxieStreams] Found default subdomain: {default_subdomain}", xbmc.LOGINFO)
                    ...
                    for func_name, func_body in func_matches:
                        ...
                        debug_log(f"[RoxieStreams] Found JS function: {func_name}", xbmc.LOGINFO)
                
                for idx, button in enumerate(buttons, 1):
                    onclick = button.get("onclick", "")
                    button_text = button.get_text(strip=True) or f"Stream {idx}"
                    debug_log(f"[RoxieStreams] Button '{button_text}' onclick: {onclick}", xbmc.LOGINFO)
                    ...
                    if direct_url_match:
                        ...
                        debug_log(f"[RoxieStreams] Button '{button_text}' (direct URL): {stream_url}", xbmc.LOGINFO)
                        ...
                        if func_match:
                            ...
                            debug_log(f"[RoxieStreams] Button '{button_text}' looking for {func_name} URL", xbmc.LOGINFO)
                            ...
                            if func_name in js_functions:
                                ...
                                if random_match:
                                    ...
                                    js_url = chosen_url
                                    js_fallbacks = self._build_domain_fallbacks(chosen_url, button_subdomain, stream_path, domains_list)
                                    debug_log(f"[RoxieStreams] Button '{button_text}' using getRandomStream: {js_url} ({len(js_fallbacks)} fallbacks)", xbmc.LOGINFO)
                                else:
                                    ...
                                    if var_match:
                                        ...
                                        for script in scripts:
                                            ...
                                            if var_def:
                                                ...
                                                debug_log(f"[RoxieStreams] Button '{button_text}' found variable {var_name}: {js_url}", xbmc.LOGINFO)
                                                break
                                    else:
                                        ...
                                        for m in matches:
                                            if m.startswith("http"):
                                                js_url = m
                                                debug_log(f"[RoxieStreams] Button '{button_text}' found URL in {func_name}: {js_url}", xbmc.LOGINFO)
                                                break
                            else:
                                debug_log(f"[RoxieStreams] Button '{button_text}' function {func_name} not found in js_functions", xbmc.LOGWARNING)
                            if js_url:
                                js_url, headers, inputstream = self._process_stream_url(js_url)
                                patched_url, _ = self._patch_m3u8_segments(js_url, headers, fallback_urls=js_fallbacks if js_url == chosen_url else None)
                                ...
                                debug_log(f"[RoxieStreams] Button '{button_text}' using fallback subdomain: {default_subdomain}", xbmc.LOGINFO)
                                ...
                                fallbacks = self._build_domain_fallbacks(chosen_url, default_subdomain, stream_path, domains_list)
                                chosen_url, headers, inputstream = self._process_stream_url(chosen_url)
                                patched_url, _ = self._patch_m3u8_segments(chosen_url, headers, fallback_urls=fallbacks)
                                links.append(JetLink(address=patched_url, name=button_text, resolveurl=False, inputstream=inputstream, headers=headers, direct=True))
                        ...
                        if iframe_match:
                            ...
                            debug_log(f"[RoxieStreams] Button '{button_text}' (iframe/raw): {raw_url}", xbmc.LOGINFO)
                            try:
                                raw_link = self._resolve_raw_page(raw_url, button_text)
                                if raw_link is not None and raw_link.address:
                                    links.append(raw_link)
                                else:
                                    debug_log(f"[RoxieStreams] Button '{button_text}' raw page yielded no link", xbmc.LOGWARNING)
                            except Exception as e:
                                debug_log(f"[RoxieStreams] Button '{button_text}' iframe resolve failed: {e}", xbmc.LOGWARNING)
                    else:
                        ...
                        if stream_match:
                            ...
                            debug_log(f"[RoxieStreams] Button '{button_text}' subdomain: {button_subdomain}", xbmc.LOGINFO)
                            
                            chosen_url = self._pick_working_stream_url(button_subdomain, stream_path, domains_list, button_text)
                            fallbacks = self._build_domain_fallbacks(chosen_url, button_subdomain, stream_path, domains_list)
                            chosen_url, headers, inputstream = self._process_stream_url(chosen_url)
                            patched_url, _ = self._patch_m3u8_segments(chosen_url, headers, fallback_urls=fallbacks)
                            links.append(JetLink(address=patched_url, name=button_text, resolveurl=False, inputstream=inputstream, headers=headers, direct=True))
        except Exception as e:
            debug_log(f"[RoxieStreams] Exception in get_links: {e}", xbmc.LOGERROR)
        ...
        if links:
            ...
            for link in links:
                ...
                if is_mpd or is_dazn:
                    ...
                    if lic_type == "org.w3.clearkey" and lic_key:
                        ...
                        debug_log(f"[RoxieStreams] Keeping ClearKey MPD stream: {link.name} (license_key={lic_key})", xbmc.LOGINFO)
                    else:
                        debug_log(f"[RoxieStreams] Skipping DRM stream: {link.name} -> {link.address[:80]}... (license_type={lic_type})", xbmc.LOGINFO)
                    continue
                filtered.append(link)
            ...
            if not links:
                debug_log(f"[RoxieStreams] All links were DRM, returning empty", xbmc.LOGWARNING)
                return []
            ...
            debug_log(f"[RoxieStreams] Returning {len(ordered)} links (all streams available)", xbmc.LOGINFO)
            return ordered
        # Fallback to single link extraction
        debug_log(f"[RoxieStreams] No stream buttons found, falling back to get_link", xbmc.LOGINFO)
        return [self.get_link(url)]
    
    def get_link(self, url: JetLink) -> JetLink:
        debug_log(f"[RoxieStreams] get_link called for: {url.address}", xbmc.LOGINFO)
        ...
        try:
            ...
            if buttons:
                ...
                for script in scripts:
                    ...
                    if subdomain_match:
                        ...
                        debug_log(f"[RoxieStreams] Found subdomain: {subdomain}", xbmc.LOGINFO)
                first_button = buttons[0]
                onclick = first_button.get("onclick", "")
                debug_log(f"[RoxieStreams] First button onclick: {onclick}", xbmc.LOGINFO)
                
                # Try new format first: showPlayer('clappr', 'https://601.833577.xyz/daffodil.m3u8')
                direct_url_match = re.search(r"showPlayer\([^,]+,\s*['\"]([^'\"]+\.(?:m3u8|mpd)[^'\"]*)['\"]", onclick)
                if direct_url_match:
                    stream_url = direct_url_match.group(1)
                    debug_log(f"[RoxieStreams] Found direct m3u8 URL: {stream_url}", xbmc.LOGINFO)
                ...
                if stream_match:
                    ...
                    # Use first domain as primary, rest as fallbacks
                    primary_url = f"https://{button_subdomain}.{domains_list[0]}/{stream_path}"
                    fallback_urls = [f"https://{button_subdomain}.{d}/{stream_path}" for d in domains_list[1:]]
                    debug_log(f"[RoxieStreams] Trying: {primary_url} ({len(fallback_urls)} fallbacks)", xbmc.LOGINFO)
                    stream_url, headers, inputstream = self._process_stream_url(primary_url)
                    patched_url, _ = self._patch_m3u8_segments(stream_url, headers, fallback_urls=fallback_urls)
                    return JetLink(address=patched_url, inputstream=inputstream, resolveurl=False, headers=headers, direct=True)
            ...
            if link is not None:
                ...
                debug_log(f"[RoxieStreams] scan_page succeeded: {getattr(link, 'address', None)}", xbmc.LOGINFO)
                return link
            ...
            for script in scripts:
                ...
                if clappr_match:
                    ...
                    debug_log(f"[RoxieStreams] Found Clappr m3u8: {stream_url}", xbmc.LOGINFO)
                    stream_url, headers, inputstream = self._process_stream_url(stream_url)
                ...
                if found:
                    stream_url = found[0]
                    debug_log(f"[RoxieStreams] Found m3u8 in script: {stream_url}", xbmc.LOGINFO)
                    stream_url, headers, inputstream = self._process_stream_url(stream_url)
                    return JetLink(address=stream_url, inputstream=inputstream, resolveurl=False, headers=headers, direct=True)
                ...
                if videojs_match:
                    ...
                    debug_log(f"[RoxieStreams] Found Video.js/JW m3u8: {stream_url}", xbmc.LOGINFO)
                    stream_url, headers, inputstream = self._process_stream_url(stream_url)
                    return JetLink(address=stream_url, inputstream=inputstream, resolveurl=False, headers=headers, direct=True)
                # Base64-decoded URLs
                    ...
                    try:
                        ...
                        if 'm3u8' in decoded or 'mpd' in decoded:
                            ...
                            if m3u8_url:
                                ...
                                debug_log(f"[RoxieStreams] Decoded b64 m3u8: {stream_url}", xbmc.LOGINFO)
                                stream_url, headers, inputstream = self._process_stream_url(stream_url)
                                return JetLink(address=stream_url, inputstream=inputstream, resolveurl=False, headers=headers, direct=True)
                    except:
                        pass
            ...
            if iframe and iframe.get("src"):
                ...
                if "youtube.com/live_chat" in iframe_url:
                    debug_log(f"[RoxieStreams] Skipping YouTube chat iframe: {iframe_url}", xbmc.LOGINFO)
                else:
                    debug_log(f"[RoxieStreams] Found iframe src: {iframe_url}", xbmc.LOGINFO)
                        ...
                        if re.search(r"\.js", m3u8_text):
                            ...
                            for line in lines:
                                if line.strip().endswith('.js') and not line.startswith('#') and '.ts' not in line:  # Assume it's a segment
                                    ...
                                    debug_log(f"[RoxieStreams] Patched segment: {line.strip()} -> {patched_line.strip()}", xbmc.LOGINFO)
                                else:
                            ...
                            for line in patched.splitlines():
                                if line.strip().endswith('.ts') and not line.startswith('http'):
                                    ...
                                    try:
                                        ...
                                        debug_log(f"Resolved segment {line.strip()}: {resp.status_code} ({full_seg})", xbmc.LOGINFO)
                                    except Exception as e:
                                        debug_log(f"Resolved segment {line.strip()}: ERROR {e}", xbmc.LOGERROR)
                        
                        if hasattr(iframe_link, 'resolveurl'):
                            iframe_link.resolveurl = False
                        # Log playlist
                        with open(iframe_link.address, "r", encoding="utf-8") as logf:
                            debug_log("--- M3U8 Playlist Contents ---", xbmc.LOGINFO)
                            debug_log(logf.read(), xbmc.LOGINFO)
                        debug_log(f"[RoxieStreams] Returning iframe_link: {getattr(iframe_link, 'address', None)}", xbmc.LOGINFO)
                        return iframe_link
        except Exception as e:
            debug_log(f"[RoxieStreams] Exception in get_link: {e}", xbmc.LOGERROR)
        ...
        #         debug_log(f"[RoxieStreams] Proxy found m3u8: {resp.text.strip()}", xbmc.LOGINFO)
        #         return JetLink(address=resp.text.strip(), resolveurl=False)
        #     else:
        #         debug_log(f"[RoxieStreams] Proxy failed: status {resp.status_code}, response: {resp.text}", xbmc.LOGERROR)
        # except Exception as e:
