from ..models import *
from ..endpoints import FCTV_SCRAPE
...
class _FCTVProxyHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        debug_log(f"[FCTV] Proxy GET {self.path}", xbmc.LOGINFO)
        ...
        if raw_path_for_token.startswith("fctv/") and raw_path_for_token.endswith(".m3u8"):
            ...
            if entry.get("cache") and (now - entry.get("cache_time", 0)) < 1.0:
                ...
                try:
                    ...
                    debug_log(f"[FCTV] Upstream response: {resp.status_code}", xbmc.LOGINFO)
                    debug_log(f"[FCTV] Upstream content-type: {resp.headers.get('Content-Type', 'unknown')}", xbmc.LOGINFO)
                    debug_log(f"[FCTV] Upstream content-encoding: {resp.headers.get('Content-Encoding', 'none')}", xbmc.LOGINFO)
                    if resp.status_code != 200:
                        debug_log(f"[FCTV] Upstream error {resp.status_code}: {resp.text[:200]}", xbmc.LOGWARNING)
                    ...
                    debug_log(f"[FCTV] Upstream raw bytes: {len(raw_bytes)}", xbmc.LOGINFO)
                    ...
                    try:
                        ...
                        if raw_bytes[:2] == b'\x1f\x8b':
                            debug_log("[FCTV] Detected gzip, decompressing...", xbmc.LOGINFO)
                            raw_bytes = gzip.decompress(raw_bytes)
                        # Check if it's zlib/deflate
                        elif raw_bytes[:2] == b'\x78\x9c' or raw_bytes[:2] == b'\x78\x01' or raw_bytes[:2] == b'\x78\xda':
                            debug_log("[FCTV] Detected zlib/deflate, decompressing...", xbmc.LOGINFO)
                            raw_bytes = zlib.decompress(raw_bytes)
                    except Exception as decomp_err:
                        debug_log(f"[FCTV] Decompression attempt failed: {decomp_err}", xbmc.LOGDEBUG)
                    try:
                    ...
                    debug_log(f"[FCTV] Upstream body length: {len(body)}", xbmc.LOGINFO)
                    debug_log(f"[FCTV] Upstream body first 100 chars: {repr(body[:100])}", xbmc.LOGINFO)
                    if not body or "#EXTM3U" not in body:
                        debug_log(f"[FCTV] Upstream body invalid: {body[:200]}", xbmc.LOGWARNING)
                    ...
                    debug_log(f"[FCTV] Rewrote m3u8, {len(rewritten)} lines, {len(data)} bytes", xbmc.LOGINFO)
                except Exception as e:
                    debug_log(f"[FCTV] manifest rebuild failed: {e}", xbmc.LOGWARNING)
            ...
            try:
                ...
                debug_log(f"[FCTV] Sent m3u8 response ({len(data)} bytes)", xbmc.LOGINFO)
            except (ConnectionAbortedError, BrokenPipeError) as e:
                debug_log(f"[FCTV] client disconnected during manifest write: {e}", xbmc.LOGDEBUG)
            return
        ...
        if raw_path_for_token.startswith("fctv/seg/"):
            ...
            if not entry or not seg_path:
                debug_log(f"[FCTV] Segment token/path not found: {token}/{seg_path}", xbmc.LOGWARNING)
            ...
            debug_log(f"[FCTV] Proxy segment: {target}", xbmc.LOGINFO)
            try:
                ...
                debug_log(f"[FCTV] Segment upstream status: {upstream_resp.status_code}", xbmc.LOGINFO)
                ...
                try:
                    ...
                    debug_log(f"[FCTV] Segment sent {bytes_sent} bytes", xbmc.LOGINFO)
                    try:
                        upstream_resp.close()
                    except Exception:
                        pass
    ...
    def _fail(self, code: int, body: bytes) -> None:
        try:
            ...
            debug_log(f"[FCTV] _fail write error: {e}", xbmc.LOGDEBUG)
...
def _ensure_proxy() -> int:
    with _PATCH_PROXY["lock"]:
        ...
        debug_log(f"[FCTV] Patch proxy listening on 127.0.0.1:{port}", xbmc.LOGINFO)
        return port
...
class FCTV(JetExtractor):
    ...
    def _refresh(self) -> None:
        ...
        try:
            ...
            debug_log(f"[FCTV] Failed to fetch scrape JSON: {e}", xbmc.LOGWARNING)
    ...
    def _prefetch(self, token: str) -> None:
        ...
        def _do():
            try:
                ...
                debug_log(f"[FCTV] prefetch failed: {e}", xbmc.LOGDEBUG)
    ...
    def _normalize(self, entry: dict) -> Optional[JetItem]:
        ...
        for l in entry.get("links") or []:
            ...
            if not upstream_headers:
                upstream_headers = {}
            debug_log(f"[FCTV] Scraped headers for {addr}: {upstream_headers}", xbmc.LOGINFO)
            ...
            debug_log(f"[FCTV] Merged headers: {upstream_headers}", xbmc.LOGINFO)
            ...
            try:
                ...
                debug_log(f"[FCTV] proxy registration failed, using raw URL: {e}", xbmc.LOGWARNING)
                proxy_url = addr
            ...
            debug_log(f"[FCTV] Creating JetLink with proxy_url={proxy_url}, headers={link_headers}", xbmc.LOGINFO)
            links.append(
                JetLink(
                    address=proxy_url,
                    name=name,
    ...
    def get_items(self, params: Optional[dict] = None, progress: Optional[JetExtractorProgress] = None) -> List[JetItem]:
        ...
        debug_log(f"[FCTV] Total items: {len(items)}", xbmc.LOGINFO)
        return items
